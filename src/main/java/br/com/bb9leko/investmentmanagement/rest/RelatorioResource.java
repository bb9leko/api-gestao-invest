...
import br.com.bb9leko.investmentmanagement.dto.AtivoConsolidadoDTO;
...
public class RelatorioResource {
    ...
    public List<AtivoConsolidadoDTO> listarAtivosAgrupados() {
        // Refatorado: delega para service utilitário
        return AtivoConsolidadoService.consolidar(transacaoRepository.listAll());
    }

    // Nova classe utilitária interna para consolidar ativos
    static class AtivoConsolidadoService {
        static List<AtivoConsolidadoDTO> consolidar(List<Transacao> transacoes) {
            Map<String, List<Transacao>> agrupados = transacoes.stream()
                    .collect(Collectors.groupingBy(AtivoConsolidadoService::agrupamentoChave));
            List<AtivoConsolidadoDTO> ativos = agrupados.values().stream()
                    .map(AtivoConsolidadoService::consolidarAtivo)
        ...
        private static String agrupamentoChave(Transacao t) {
        ...
        private static AtivoConsolidadoDTO consolidarAtivo(List<Transacao> transacoes) {
            ...
            for (Transacao t : transacoes) {
                ...
                if (evento == Evento.COMPRA || evento == Evento.BONIFICACAO || evento == Evento.DESDOBRAMENTO || evento == Evento.CONVERSAO) {
                    saldo = saldo.add(qtd);
